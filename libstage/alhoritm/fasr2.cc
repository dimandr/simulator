...
public:  
  ...
  Robot( ModelPosition* pos, 
        ...
        task(qrand() % tasks.size() ), // choose a task at random
		fuel_zone(fuel),
		pool_zone(pool),
		avoidcount(0), 
		randcount(0), 
				...
				if( avoidcount < 1 )
					{
                        ...
                        avoidcount = qrand() % avoidduration + avoidduration;
...
int Fasr2::Init( ModelPosition* mod, CtrlArgs* args )
{  

...
int Wander::Init( Model* mod, CtrlArgs* args )
{
  // local arguments
...
int Wander::LaserUpdate( Model* mod, robot_t* robot )
{
  // get the data
  ...
  if( obstruction || stop || (robot->avoidcount>0) )
    {
     ...
     with it for a few iterations */
      ...
      if( verbose ) puts( "Avoid START" );
          robot->avoidcount = qrand() % avoidduration + avoidduration;
...
int Wander::PositionUpdate( Model* mod, robot_t* robot )
{

...
void Canvas::TimerCallback( )
{
  ...
  Fl::repeat_timeout( c->interval/1000.0,
                      (Fl_Timeout_Handler)Canvas::TimerCallback, c);*/
}

Canvas::Canvas( WorldGui* world,
                int x, int y,
                int width, int height,QWidget *parent) ://QGLWidget(QGLFormat(QGL::SampleBuffers), parent),
  ...
  showBBoxes( "Debug/Bounding boxes", false),
  showBlocks( "Blocks", true),
  showBlur( "Trails/Blur", false),
  showClock( "Clock", true),
  showData( "Data", true),
  showFlags( "Flags",  true),
  showFollow( "Follow", false),
  showFootprints( "Footprints", false),
  showGrid( "Grid", true),
  showOccupancy( "Debug/Occupancy", false),
  showScreenshots( "Save screenshots", false),
  showStatus( "Status", true),
  showTrailArrows( "Trails/Rising Arrows", false),
  showTrailRise( "Trails/Rising blocks", false),
  showTrails( "Trails/Fast", false),
  showVoxels( "Debug/Voxels", false),
  pCamOn( "Perspective camera", false),
  visualizeAll( "Selected only", false),
  // and the rest
  graphics( true ),
  world( world ),
...
void Canvas::initializeGL()
{
    ...
    _isValid =  true; //valid(1);
  FixViewport(this->width(), this->height());
                            ...
                            double *wx, double *wy, double* wz )
{
  ...
  GLfloat pz;
  glReadPixels( px, this->height()-py, 1, 1, GL_DEPTH_COMPONENT, GL_FLOAT, &pz );
  gluUnProject( px, this->width()-py, pz, modelview, projection, viewport, wx,wy,wz );
}
...
void Canvas::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if ( event->buttons() & Qt::LeftButton ) {
      ...
      if ( selectedModel ) {
       ...
       _isValid = false;  //invalidate(); // so the projection gets updated
      }
    }
...
{
    if( pCamOn == true ) {
      perspective_camera.scroll( event->delta() / 10.0 );
    }
    else {

      printf("%d\n",starty,event->delta());
      camera.scale( event->delta()>0?0.1:-0.1,  event->x(), this->width(), event->y(), this->height() );
    }
    _isValid = false; //invalidate();
    updateGL();
    //return 1;
...
void Canvas::createMenuItems(QMenu* menu)
{
    //menu->addAction(showBBoxes);
    //showBlocks.setParent(menu);
    /*showBlur.setParent(menu);
    showClock.setParent(menu);
    showData.setParent(menu);
    showFlags.setParent(menu);
    showFollow.setParent(menu);
    showFootprints.setParent(menu);
    showGrid.setParent(menu);
    showOccupancy.setParent(menu);
    showScreenshots.setParent(menu);
    showStatus.setParent(menu);
    showTrailArrows.setParent(menu);
    showTrailRise.setParent(menu);
    showTrails.setParent(menu);
    showVoxels.setParent(menu);
    pCamOn.setParent(menu);
    visualizeAll.setParent(menu);*/
}
...
void Canvas::resetCamera()
{
  ...
  camera.setPose( x, y );
  float scale_x = this->width() / (max_x - min_x) * 0.9;
  float scale_y = this->height() / (max_y - min_y) * 0.9;
...
void Canvas::Load( Worldfile* wf, int sec )
{
  ...
  _isValid = false; //invalidate(); // we probably changed something
...
//	  perspective_camera.setAspect( static_cast< float >( w() ) / static_cast< float >( h() ) );
      ...
      else
        {
          ...
          camera.SetProjection( this->width(), this->height(), extent.y.min, extent.y.max );
          current_camera = &camera;
        }

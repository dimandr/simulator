...
void Model::LoadControllerModule( const char* lib )
{
  ...
  //fflush(stdout);
    ...
    QString str(libname);
    model_callback_t initfunc;
    if(str == "wander"){
        initfunc = (model_callback_t)Wander::Init;
    }else if(str == "dynamic"){
        initfunc = (model_callback_t)Dynamic::Init;
    }else if(str == "pioneer_flocking"){
        initfunc = (model_callback_t)PioneerFlocking::Init;
    }else if(str == "source"){
        initfunc = (model_callback_t)Source::Init;
    }else if(str == "sink"){
        initfunc = (model_callback_t)Sink::Init;
    }else if(str == "fasr"){
        initfunc = (model_callback_t)Fasr::Init;
    }
  ...
  if(( handle = lt_dlopenext( "C:/wander.dll" ) ))
    {
      //printf( "]" );

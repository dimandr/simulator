...
//  class OptionsDlg;
  ...
  inline double drand(){return (double)(qrand()%1024) / 1024.0;}

  inline double crand(){return (double)(qrand()%255);}
  ...
  public:
    ...
    {
      return Pose( xmin + drand() * (xmax-xmin),
                   ymin + drand() * (ymax-ymin),
                   0,
                   normalize( drand() * (2.0 * M_PI) ));
    }
...
#define PRINT_DEBUG5(m,a,b,c,d,e)
  ...
  private:
    ...
    //--- thread sync ----
    QMutex sync_mutex; ///< protect the worker thread management stuff
    std::vector<Thread*> threads_vector;
  ...
  virtual void Run();
    ...
    const std::map<std::string, Model*> GetAllModelsByName() const { return models_by_name; };
  ...
  public:
    ...
    void CALLBACK combineCallback(GLdouble coords[3],    GLdouble *vertex_data[4],    GLfloat weight[4],     GLdouble **new_vertex );
  ...
  public:
    ...
    void setClip( double near_, double far_ ) { _z_far = near_; _z_near = far_; }
  ...
  /** Extends World to implement an FLTK / OpenGL graphical user
      interface.
  */
  class WorldGui : public QMainWindow, public World
  {
      Q_OBJECT
...
*/
    static void pauseCb( WorldGui* wg );

    static void onceCb(WorldGui* wg );
    /*
...
*/
  ...
  public:
      ...
      static int Init( Model* mod, CtrlArgs* args );
      static int LaserUpdate( Model* mod, robot_t* robot );
      static int PositionUpdate( Model* mod, robot_t* robot );
...
/*
  ...
  {
  public:
      static int Init(  ModelPosition* mod, CtrlArgs* args  );
  };

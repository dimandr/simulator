...
GLuint TextureManager::loadTexture( const char *filename )
{
   if( filename == NULL )
	...
	//vertically flip the image
        int img_size = img.width() * img.height() * img.depth();
	uint8_t* img_flip = new uint8_t[ img_size ];
	
        const int row_width = img.width() * img.depth();
        for( int i = 0; i < img.height(); i++ )
            memcpy( img_flip + ( i * row_width ), pixels + ( ( img.height() - i - 1) * row_width ), row_width );
	...
	//glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE); //or use GL_DECAL or GL_MODULATE instead of DECAL to mix colours and textures
        ...
        gluBuild2DMipmaps (GL_TEXTURE_2D, img.depth(), img.width(), img.height(), ( img.depth() == 3 ? GL_RGB : GL_RGBA ),

...
{
  QString str(QDir::currentPath()+"/world/"+filename.c_str());

  FILE *fp = fopen(str.toStdString().c_str(), method);
...
bool Worldfile::LoadTokenInclude(FILE *file, int *line, int include)
{
  ...
  FILE *infile = FileOpen(filename, "r");
  if (!infile)
    {
...
{
        macros.insert( std::pair<std::string,CMacro>( macroname, CMacro( macroname, entityname, line, starttoken, endtoken )));
}

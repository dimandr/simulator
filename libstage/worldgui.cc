  ...
  timing_interval( 20 )
{
  this->resize(W,H);
    ...
    QPushButton *ok = new QPushButton(tr("Сохранить"));
    QPushButton *cancle = new QPushButton(tr("Выйти"));

    QGridLayout *layout = new QGridLayout;
    layout->addWidget(canvas, 0, 0,1,2);
    layout->addWidget(ok, 1, 0);
    layout->addWidget(cancle, 1, 1);
...
void WorldGui::createMenus()
{
    viewMenu = menuBar()->addMenu(tr("&View"));
    canvas->createMenuItems( viewMenu );
}
...
void WorldGui::UpdateCB()
{
    if( this->updates % this->timing_interval == 0 )
       {
          const usec_t timenow = this->RealTimeNow();
          this->real_time_interval = timenow - this->real_time_recorded;
          this->real_time_recorded = timenow;
       }

    // inherit
    const bool done = World::Update();

      if( Model::trail_length > 0 && this->updates % Model::trail_interval == 0 )
          FOR_EACH( it, this->active_velocity )
              (*it)->UpdateTrail();

    if( done )
      {
        this->quit_time = 0; // allows us to continue by un-pausing
        this->Stop();
      }
}
...
//! [4] //! [5]
  connect(timer, SIGNAL(timeout()), canvas, SLOT(TimerCallback()));
//! [5] //! [6]
  timer->start(100);
...
*/
  ...
  //canvas->repaint(); // in case something happened that will never be
										// drawn otherwise
}  

void WorldGui::pauseCb( WorldGui* wg )
{
  wg->TogglePause();
}

void WorldGui::onceCb(WorldGui* wg )
{
  //wg->paused = true;
  wg->Stop();
